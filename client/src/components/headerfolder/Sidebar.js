...
function Sidebar() {
    ...
    let  categoryOptions = myItems !== undefined ? deleteDuplicates('category')
     : false;

     let  bodyLocation = myItems !== undefined ? deleteDuplicates('body_location')
     : false;
    ...
    const filter = (attr) =>{
        ...
        return function(itemList, id){
            dispatch(filterItems(itemList.filter(item => { 
                return item[attr] === id })))
            }
     }
            ...
            <NavMenuItems>
               ...
               ? <ShowFilters/>
                ...
                <div style={{ marginTop:'50px'}}>
                    <p>Nothing to look here buddy.<br/>
                     Please go to the home page <br/>
                     to use some cool filters</p>
                </div>
                )}
...
const navStyles = (isActive) =>{
    const [leftValue, milliSecs] = isActive ? ['-2%','350ms'] : ['-100%','350ms'];
